...
// POST /api/contractor/work-orders/[id]/completion-report — Contractor submits via session
export async function POST(
  ...
  const { data: report, error: reportError } = await supabaseAdmin.from("completion_report").insert({
    work_order_id: id, contractor_id: contractorId, work_type, work_description,
  ...
  try {
    ...
    await supabaseAdmin.from("work_order_status_history").insert({
      ...
      action: "contractor_submit_completion", actor_id: contractorId, actor_role: "contractor",
    });
